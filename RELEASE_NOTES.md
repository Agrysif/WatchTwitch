...
- **Баллы канала снова считаются.** Запрос баллов падал с ошибкой в коде, а ошибка гасилась внутрь ответа — наружу выходил просто ноль, и выглядело это как «баллы не начисляются»
- **Процент дропсов у категорий больше не ноль.** Считалась доля полностью полученных наград: посмотрев 45 минут из 60 и ничего не забрав, категория показывала 0 вплоть до самого получения. Теперь виден настоящий прогресс по времени
- **Качество перестало само уползать на «Источник».** Если у канала не оказывалось нужной дорожки, приложение поднималось на ступень выше и запоминало это навсегда — а категории переключаются помногу раз за ночь. Теперь вынужденный подъём действует только на этом канале, а ваш выбор в настройках остаётся нетронутым
- **Прогресс дропсов под стримом больше не пропадает.** Панель пряталась от одного неудачного ответа Twitch
...
- **Channel points are counted again.** The points request failed on a coding error, and the failure was swallowed into the response — a plain zero came out, which looked like points simply not accruing
- **Category drop progress is no longer stuck at zero.** It counted fully claimed rewards only: watch 45 minutes out of 60 and claim nothing, and the category showed 0 right up to the moment of claiming. Real watch-time progress is shown now
- **Quality no longer creeps up to Source on its own.** When a channel lacked the requested rendition, the app stepped up and remembered that permanently — and categories switch many times over a night. The forced step now applies to that channel only, leaving your setting untouched
- **The drops panel under the stream stops disappearing.** It used to hide on a single bad response from Twitch
